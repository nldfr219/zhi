...
function module_img_visitors_per_hour($arguments)
{
    global $lvc_cache_dir;
	global $lvc_images_format;
	global $lvc_pattern_per_hour;
	global $lvc_base_img_per_hour;
    ...
    global $lvm_img_arr_months_graph, $lvm_img_per_hour;
    ...
    if (is_image_expired($file_name, $cache_delay))
	{
        ...
        $pattern = $fct_imagecreatefrom('images/'.$lvc_pattern_per_hour.'.'.$lvc_images_format);
        ...
        // title
        $title = $lvc_img_site_name.' - '.$lvm_img_per_hour.' - '.$lvm_img_arr_months_graph[(int)$month] . ' ' . $year;
        ...
        for ($cnt = 0; $cnt <= 23; $cnt++)
		{
			...
			else
			{
                ...
                $query .= "WHERE DATE LIKE '".$year."/".$month."/__ ".$hour.":__'";
                $gDb->DbQuery($query);
